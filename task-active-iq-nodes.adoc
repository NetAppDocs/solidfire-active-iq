...
|Platform Config Version | The config version of the chassis you are running on. 
|BIOS Version | The BIOS version for your node's hardware.
|BMC Firmware Revision | The revision of the firmware currently running on the Baseboard Management Controller (BMC).
...
|End of Availability |Date when nodes are no longer available for purchase, but are still supported.
|End of Hardware Support |Date when hardware is no longer supported.
|End of Software Support |Date when software is no longer supported.
|===
