...
== Node Utilization
On the *Node Utilization* page of the *Reporting* drop-down menu for a selected cluster, each node can be selected for view. When a node is selected, it shows its utilization as a percentage at a certain time on the graph. This uses the default time range selectors that other graphs use. The parameter used to show this is `recentPrimaryTotalHeat`.
...
== Events
On the *Events* page of the *Reporting* drop-down menu for a selected cluster, you can select between the *Events* and *gcEvents* tabs to view information about key events and gc events that have occurred on the cluster. To view gc events, you must use the gcEvents tab. This information can be filtered and exported to a CSV file.
