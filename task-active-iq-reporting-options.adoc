...
== Node Utilization
On the *Node Utilization* page of the *Reporting* drop-down menu for a selected cluster, you can select and view each node. 

Beginning with Element 12.8, node utilization information is available as `nodeHeat` using the `GetNodeStats` and `ListNodeStats` API methods. The `nodeHeat` object is a member of the `nodeStats` object and displays node utilization information based on the ratio of primary total IOPS or total IOPS to the configured IOPS averaged over time. The node utilization graph shows this as a percentage derived from the metric `recentPrimaryTotalHeat`.
...
== Events
On the *Events* page of the *Reporting* drop-down menu for a selected cluster, you can select between the *Events* and *gcEvents* tabs to view information about key events that have occurred on the cluster. By default, selecting *Events* shows all of the events except for gcEvents to improve readability. In order to see all events, including gcEvents, select the tab labeled *gcEvents*. This information can be filtered and exported to a CSV file.
...
|Type |The type of event being logged, for example, API events, clone events, or gc events. See the https://docs.netapp.com/us-en/element-software/index.html[Element Software documentation^] for more information.
