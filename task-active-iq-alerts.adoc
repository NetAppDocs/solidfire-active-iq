...
+
NOTE: Only one suppression type is allowed at a given time. To change the type for a cluster with an existing suppression, first select *Override Existing* and then select the new suppression type.
...
. From the *Dashboard* overview or *Alerts* > *Cluster Suppression*, you can end suppression for the single or multiple clusters you want to resume normal alert reporting:
