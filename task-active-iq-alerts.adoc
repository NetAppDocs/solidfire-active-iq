...
== Alert notification email
Subscribers to SolidFire Active IQ alerts receive status emails for any alert that triggers on the system. There are three types of status emails associated with alerts:
[cols=2*,cols="35,65"]
|===
...
NOTE: Multiple notifications are grouped into a single email with the most severe faults at the top of the email similar to the following example:


image:multiple_emails_example.png[Example email]
